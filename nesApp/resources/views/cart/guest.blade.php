        ...
        $(document).ready(function() {
            ...
            function displayAppliedCoupons() {
                ...
                appliedCoupons.forEach(coupon => {
                    const discountValue = parseFloat(coupon.discount);
                    couponList.append(
                        `<li>${coupon.code}: ${discountValue <= 100 ? discountValue + '%' : '$' + discountValue}</li>`
                    );
                });
            }
            ...
            $('#couponForm').on('submit', function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#result').html(
                            '<div class="alert alert-success">Coupon code <strong>' +
                            response.data.code + '</strong> applied! Discount: ' + (
                    ...
                    error: function(xhr) {
                        const message = xhr.responseJSON ? xhr.responseJSON.message :
                            'An error occurred.';
                        $('#result').html('<div class="alert alert-danger">' + message +
                            '</div>');
                    }
                });
            });
